...
def collect_client_diff_weights_pt(client_flex_model, *args, **kwargs):
    ...
    with torch.no_grad():
        ...
        #print("State dict:", client_flex_model["model"].state_dict().values().shape)
        #for val in client_flex_model["model"].state_dict().values():
        #    print(val.shape)
        n_prm = torch.cat([val.reshape((-1, 1)) for val in client_flex_model["model"].state_dict().values()], dim=0)
        #print(n_prm.shape)
        idx=0
        for val in client_flex_model["model"].state_dict().values():
            #print("Desde Numel+Idx", idx,"Hasta Numel", idx + torch.numel(val))
            modif_param = n_prm[idx: (idx + torch.numel(val))]
            #print(modif_param.reshape(tuple(val.size())).shape)
            #print(torch.numel(val))
            idx += torch.numel(val)
            #print("idx", idx)

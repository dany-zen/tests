...
from my_models_attacks.model_extration_utils.find_witnesses import do_better_sweep
from my_models_attacks.model_extration_utils.hyperplane_normal import get_ratios_lstsq

from my_models_attacks.model_extration_utils.utils import AcceptableFailure, GatherMoreData, matmul, KnownT, cheat_get_inner_layers, which_is_zero
...
def process_block(ratios, other_ratios):
    ...
    close = differences < globals.BLOCK_ERROR_TOL * jnp.log(ratios.shape[1])

    pairings = jnp.sum(close, axis=2) >= max(globals.MIN_SAME_SIZE, globals.BLOCK_MULTIPLY_FACTOR*(np.log(ratios.shape[1])-2))
...
def graph_solve(all_ratios, all_criticals, expected_neurons, LAYER, debug=False):
    ...
    while previous_num_components > len(sorted_components):
        ...
        results = globals.pool[0].map(ratio_normalize, datas)
    ...
    for c_count, component in enumerate(sorted_components):
        ...
        if np.any(np.isnan(guessed_row)) and c_count < expected_neurons:
            print("Got NaN, need more data",len(component)/sum(map(len,components)),1/globals.sizes[LAYER+1])
            if len(component) >= 3:
                if c_count < expected_neurons:
    ...
    if len(resulting_rows)+skips_because_of_nan < expected_neurons and len(all_ratios) < globals.DEAD_NEURON_THRESHOLD:
...
def gather_ratios(critical_points, known_T, check_fn, LAYER, COUNT):
    ...
    for point in critical_points:
        print("Tiene al menos un punto", type(point))
        if LAYER > 0:
        ...
        tmp = globals.query_count
        for EPS in [globals.GRAD_EPS, globals.GRAD_EPS/10, globals.GRAD_EPS/100]:
            try:
                normal = get_ratios_lstsq(LAYER, [point], [range(globals.DIM)], known_T, eps=EPS)[0].flatten()
...
def compute_layer_values(critical_points, known_T, LAYER):
    if LAYER == 0:
        COUNT = globals.neuron_count[LAYER+1] * 3
    else:
        COUNT = globals.neuron_count[LAYER+1] * np.log(globals.sizes[LAYER+1]) * 3


    # type: [(ratios, critical_point)]
    this_layer_critical_points = []
    print("Que son los puntos criticos a ver", type(critical_points))
    ...
    while True:
        ...
        print("Query count after that search:", globals.query_count)
        ...
        try:
            ...
            COUNT = globals.neuron_count[LAYER+1]
    ...
    extracted_bias = [matmul(known_T.forward(point_per_class[i], with_relu=True), extracted_normals[:,i], c=None) for i in range(globals.neuron_count[LAYER+1])]
    ...
    if globals.CHEATING:
        ...
        real_scaled = globals.A[LAYER]/globals.A[LAYER][0]
        ...
        for i in range(len(extracted_bias)):
            ...
            mask.append((globals.A[LAYER][0,which_idx]))
    
        print('matrix norm difference', np.sum(np.abs(extracted_normals*mask - globals.A[LAYER][:,reorder_rows])))
    else:

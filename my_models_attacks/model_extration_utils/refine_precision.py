...
from my_models_attacks.model_extration_utils.utils import matmul, which_is_zero
...
def improve_row_precision(args):
    ...
    def get_more_points(NUM):
        ...
        while len(critical_points) <= NUM:
            ...
            points = np.random.normal(0, 1e3, size=(100,globals.DIM,))
    ...
    for _ in range(1):
        ...
        if globals.CHEATING:
            out = np.abs(matmul(hidden_layer, globals.A[LAYER],globals.B[LAYER]))
    ...
    if globals.CHEATING:
        other = globals.A[LAYER][:,which_neuron]
...
def improve_layer_precision(LAYER, known_T, known_A, known_B):
    ...
    out = map(improve_row_precision,
              [(LAYER, known_T, known_A, known_B, row, False) for row in range(globals.neuron_count[LAYER+1])])
    new_A, new_B = zip(*out)

...
from my_models_attacks.model_extration_utils.utils import run, get_polytope_at, get_hidden_at, AcceptableFailure, KnownT, matmul, cheat_get_inner_layers, which_is_zero
from my_models_attacks.model_extration_utils.hyperplane_normal import get_ratios_lstsq, get_ratios
...
def is_on_following_layer(known_T, known_A, known_B, point):
    ...
    normal = get_ratios([point], [range(globals.DIM)], eps=globals.GRAD_EPS)[0].flatten()
    normal = normal / np.sum(normal**2)**.5

    for tol in range(10):
    
        random_dir = np.random.normal(size=globals.DIM)
...
def find_plane_angle(known_T,
    ...
    while True:
        x_dir_base = np.sign(np.random.normal(size=globals.DIM))/globals.DIM**.5
        y_dir_base = np.sign(np.random.normal(size=globals.DIM))/globals.DIM**.5
        # When the input dimension is odd we can't have two orthogonal
        # vectors from {-1,1}^DIM
        if np.abs(np.dot(x_dir_base, y_dir_base)) <= globals.DIM%2 + 1e-8:
            break
...
def follow_hyperplane(LAYER, start_point, known_T, known_A, known_B,
    ...
    while True:
        ...
        try:
            ...
            perp_dir = get_ratios_lstsq(0, [start_point], [range(globals.DIM)], KnownT([], []), eps=1e-5)[0].flatten()
        ...
        while success is None:
            if start_box_step < 0:
                ...
                current_change_axis = np.random.randint(0, globals.sizes[LAYER+1])
...
def solve_contractive_sign(known_T, weight, bias, LAYER):
    ...
    hidden = np.zeros((globals.sizes[LAYER+1]))
...
def solve_layer_sign(known_T, known_A0, known_B0, critical_points, LAYER,
    ...
    print("Query count", globals.query_count)
        

    K = globals.neuron_count[LAYER+1]
    MAX = (1<<K)
    if already_checked_critical_points:
        bounds = [(MAX-1, MAX)]
    else:
    ...
    all_res = globals.pool[0].map(is_solution_map, [(bound, extra_args_tup) for bound in bounds])

...
from my_models_attacks.model_extration_utils.global_vars import __cheat_A, __cheat_B
import traceback
...
def run(x,inner_A= globals.__cheat_A, inner_B= globals.__cheat_B):#
    """
    ...
    """
    global query_count
    if len(inner_A) == 0 and len(inner_B) == 0:
        inner_A = globals.__cheat_A
        inner_B = globals.__cheat_B
    print(len(inner_A), len(inner_B))
    globals.query_count += x.shape[0]
    print("El contador de colas", globals.query_count)
    ...
    if TRACK_LINES:
        for line in traceback.format_stack():
            ...
            if line_no not in globals.query_count_at:
                globals.query_count_at[line_no] = 0
            globals.query_count_at[line_no] += x.shape[0]
...
def cheat_get_inner_layers(x,A=globals.A,B=globals.B, as_list=False):
    if not globals.CHEATING: raise NoCheatingError()
    return _cheat_get_inner_layers(x,A,B,as_list)

def _cheat_get_polytope_id(x,A=__cheat_A,B=__cheat_B, as_list=False, flatten=True):#ESTOOOOO VERLOOOOOOOO
    """
...
def cheat_get_polytope_id(x,A=globals.A,B=globals.B, as_list=False, flatten=False):
    if not globals.CHEATING: raise NoCheatingError()
...
def check_quality(layer_num, extracted_normal, extracted_bias, do_fix=False):
    ...
    reorder = [None]*(globals.neuron_count[layer_num+1])
    for i in range(globals.neuron_count[layer_num+1]):
        gaps = []
        ratios = []
        for j in range(globals.neuron_count[layer_num+1]):
        ...
        if do_fix and globals.CHEATING:

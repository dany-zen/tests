...
def construct_dataloaders(dataset, defs, data_path='~/data', shuffle=True, normalize=True):
    ...
    if dataset == 'CIFAR10':
        trainset, validset, mean,  std = _build_cifar10(path, defs.augmentations, normalize)
        print(type(validset))
    ...
    return loss_fn, trainloader, validloader, mean,  std
...
def _build_cifar10(data_path, augmentations=True, normalize=True):
    ...
    print("shape train", trainset.data.shape)

    validset = torchvision.datasets.CIFAR10(root=".", train=False, download=True, transform=transform)
    validset.data = validset.data[:1000]
    validset.targets = validset.targets[:1000]
    print("shape valid", len(validset.targets))
